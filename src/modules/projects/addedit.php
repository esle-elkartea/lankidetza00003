<?php /* PROJECTS $Id: addedit.php,v 1.94 2006/04/10 17:05:18 Attest sw-libre@attest.es Exp $ */
/* PROJECTS $Id: addedit.php,v 1.93 2005/03/31 16:42:21 jcgonz Exp $ */
...
$row = new CCompany();
$companies = $row->getAllowedRecords( $AppUI->user_id, 'company_id, company_name, company_type', 'company_name' );
...
$q->addWhere('u.user_contact = con.contact_id AND u.user_id !=1');
...
$department_selection_list = "<div id='sel'>".$AppUI->_("Departments")."<br /><select name='dept_ids[]' size='$departments_count' multiple style=''>$department_selection_list</select></div>
						<div id='but' style='visibility:hidden'><input type='button' class='button' value='".$AppUI->_("Select department...")."' onclick='javascript:popDepartment();' /><input type=\"hidden\" name=\"project_departments\"></div>";
} else {
$department_selection_list = "<div id='sel' style='visibility:hidden'>".$AppUI->_("Departments")."<br /><select name='dept_ids[]' size='$departments_count' multiple style=''>$department_selection_list</select></div>
			<div id='but'><input type='button' class='button' value='".$AppUI->_("Select department...")."' onclick='javascript:popDepartment();' /><input type=\"hidden\" name=\"project_departments\"></div>";
}
...
window.open( 'index.php?m=public&a=calendar&dialog=1&callback=setCalendar&date=' + idate, 'calwin', 'width=280, height=240, scollbars=false' );
}
...
function vaciarDepartamento () {
	document.editFrm.project_departments.value = '';
...
<?php
$row0 = new CCompany();
$extra['where']='(company_type!=3 and company_type!=2)';
$companies = $row0->getAllowedRecords( $AppUI->user_id, 'company_id, company_name, company_type', 'company_name','',$extra );
$companies = arrayMerge( array( '0'=>'' ), $companies );		
echo arraySelect( $companies, 'project_company', 'class="text" size="1" onchange="javascript:vaciarDepartamento()"', $row->project_company );
...
?> *</td>
		...
		<tr>
			<td align="right" nowrap="nowrap"><?php echo $AppUI->_('etiq_Cliente');?>:</td>
			<td width="100%" colspan="2">
				<?
                        	$row1 = new CCompany();
							$extra['where']='company_type=2';
		...
		<tr>
			<td align="right" nowrap="nowrap"><?php echo $AppUI->_('etiq_Moldista');?></td>
			<td width="100%" colspan="2">
				<?
                        	$row2 = new CCompany();
...
<tr>
	<td>
		<input class="button" type="button" name="cancel" value="<?php echo $AppUI->_('cancel');?>" onClick="javascript:location.href = './index.php?m=projects';" />
	</td>
	<td align="right">
