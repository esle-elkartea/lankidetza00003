<?php /* ADMIN $Id: vw_usr_perms.php,v 1.20 2006/04/11 17:52:59 Attest sw-libre@attest.es  Exp $ */
/* ADMIN $Id: vw_usr_perms.php,v 1.29 2005/03/28 04:41:11 ajdonnison Exp $ */
...
function delIt(id) {
	if (confirm( '<?echo $AppUI->_('Are you sure you want to delete this permission?')?>' )) {
		var f = document.frmPerms;
		f.del.value = 1;
		f.permission_id.value = id;
		f.submit();
...
<tr>
	...
	<td width="100%"><?php //inaecho arraySelect($modules, 'permission_module', 'size="1" class="text"', 'grp,all', true);?>
	
	
	
	<select name="permission_module" size="1" class="text">
			<option value="grp,12">Modulos: Administrador</option>
			<option value="grp,13">Modulos: No Admin</option>
			<option value="mod,11">Admin. Usuarios</option>
			<option value="mod,26">Admin. Roles</option>
			<option value="mod,14">Empresas</option>
			<option value="mod,15">Contactos</option>
			<option value="mod,17">Ficheros</option>
			<option value="mod,19">Ayuda</option>
			<option value="mod,20">Proyectos</option>
			<option value="mod,22">Tareas</option>
			<option value="mod,23">Submodulo Tareas: Registro de acciones</option>
			<option value="mod,27">Submodulo Usuarios: Tabla de usuarios</option>
	</select>
