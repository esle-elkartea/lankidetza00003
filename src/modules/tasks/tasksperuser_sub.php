<?php /* TASKS $Id: tasksperuser_sub.php,v 1.1 2006/03/07 18:34:16 Attest sw-libre@attest.es Exp $ */
...
$canEditINA=$perms->checkModule('admin','edit');
...
<tr>
	...
	<td>
        ...
        <td><table><tr><td>
                <?php echo $AppUI->_( 'Levels to display' ); ?></td><td>
		<select name="max_levels"  value="<?php echo $max_levels; ?>" />
		<?for($i=1;$i<=5;$i++) { ?>
			<option value=<?echo $i?> <?if ($i==$max_levels) echo 'selected'?>>  <?echo $i?>  </option>	
}		<?}?>
		</select></td></tr></table>
	</td>
</tr>
...
//echo "<pre>$sql</pre>";
	...
	if ( count($task_list) == 0 ) {
		...
		$table_header = "<tr>".
                                                "<th nowrap=\"nowrap\" ></th>".
                                                "<!--ina<th nowrap=\"nowrap\" >".$AppUI->_("P")."</th>-->".
						...
						"<th nowrap=\"nowrap\" >".$AppUI->_("End[d]")."</th>".
                                                ...
                                                if ($canEditINA) { $table_header.="<th nowrap=\"nowrap\" >".$AppUI->_("Possible Assignees")."</th>"; }
                                                //if ($perms->checkModule('admin','edit')) { $table_header.="<th nowrap=\"nowrap\" >".$AppUI->_("Possible Assignees")."</th>"; }
						$table_header.="</tr>";
		$table_rows = "";
		...
		foreach($user_list as $user_id => $user_data){
			...
			$tmpuser= "<form name=\"assFrm$user_id\" action=\"index.php?m=tasks&a=tasksperuser\" method=\"post\">
				<input type=\"hidden\" name=\"chUTP\" value=\"0\" />
					  ...
					  ."</B></font></td>";
		    for($w=1;$w<=(4+weekCells($display_week_hours,$sss,$sse));$w++) {
			...
			foreach($task_list as $task) {
				if (!isChildTask($task)) {
					if (isMemberOfTask($task_list,$Ntasks,$user_id,$task)) {
						$tmptasks.=displayTask($task_list,$task,0,$display_week_hours,$sss,$sse, $user_id,$canEditINA);
						// Get children
						$tmptasks.=doChildren($task_list,$Ntasks,
											  $task->task_id,$user_id,
											  1,$max_levels,$display_week_hours,$sss,$sse,$canEditINA);
					} else {
			...
			if (($task->task_parent==$id) and isChildTask($task)) {
				...
				if (isMemberOfTask($list,$N,$uid,$task)) {
					...
					$tmp.=doChildren($list,$N,$task->task_id,
                                     ...
                                     $display_week_hours,$ss,$se,$canEditINA);
				}
			}
		}
	}
...
function displayTask($list,$task,$level,$display_week_hours,$fromPeriod,$toPeriod, $user_id,$canEditINA) {
	...
	foreach ($users as $row) {
                if ($row["user_id"]) {
                        ...
                        }else { $us.= $sep.$row['contact_last_name']."&nbsp;(".$row['perc_assignment']."%)"; }
                        /*ina*/
                        $sep = ", ";
                }
        }
	...
	<table width="100%" border="0" cellpadding="2" cellspacing="1" class="std">
		...
		if (!$show_orphaned) {
			...
			foreach($orphTasks as $task) {
					$tmptasks.=displayTask($orphTasks,$task,0,$display_week_hours,$sss,$sse, $user_id,$canEditINA);
