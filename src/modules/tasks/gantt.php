<?php /* TASKS $Id: gantt.php,v 1.48 2006/04/18 09:51:37 Attest sw-libre@attest.es Exp $ */
...
project_name, task_dynamic, task_start_date_ir, task_end_date_ir, task_duration_ir
";
...
for ($x=0; $x < $nums; $x++) {
	...
	if ($_REQUEST["dates"]==1) {
		...
		if($row["task_start_date_ir"] == "0000-00-00 00:00:00"){
			$row["task_start_date_ir"] = date("Y-m-d H:i:s");
		}
		
		if($row["task_end_date_ir"] == "0000-00-00 00:00:00") {
			if($row["task_duration_ir"]) {
				$row["task_end_date_ir"] = db_unix2dateTime ( db_dateTime2unix( $row["task_start_date_ir"] ) + SECONDS_PER_DAY * convert2days( $row["task_duration_ir"], $row["task_duration_type"] ) );
			} else {
				$row["task_end_date_ir"] = "";
			}
		}	
	}
...
if ($showWork=='1') {
	$graph->scale->actinfo->SetColTitles(array( $AppUI->_('etiq_Tarea', UI_OUTPUT_RAW), $AppUI->_('Work', UI_OUTPUT_RAW), $AppUI->_('Inicio', UI_OUTPUT_RAW), $AppUI->_('Fin', UI_OUTPUT_RAW)),array(230,40, 60,60));
} else {
	$graph->scale->actinfo->SetColTitles(array( $AppUI->_('etiq_Tarea', UI_OUTPUT_RAW), $AppUI->_('Dur.', UI_OUTPUT_RAW), $AppUI->_('Inicio', UI_OUTPUT_RAW), $AppUI->_('Fin', UI_OUTPUT_RAW)),array(230,40, 60,60));
}
...
if ($start_date && $end_date){
        ...
        for($i = 0; $i < count(@$gantt_arr); $i++ ){
                $a = $gantt_arr[$i][0];
                //$start = substr($a["task_start_date_ir"], 0, 10);
             	//$end = substr($a["task_end_date_ir"], 0, 10);
...
for($i = 0; $i < count(@$gantt_arr); $i ++ ) {
	...
	} else if ($_REQUEST["dates"]==2) {
		$start = $a["task_start_date_ir"];
		$end_date = $a["task_end_date_ir"];
	}
...
//	$start->addDays(0);
	$start = $start->getDate();
//ina
	if ($_REQUEST["dates"]==1) $progress = 0;
	else $progress = $a["task_percent_complete"];
	...
	if($flags == "m") {		
		...
		//ina
		if ($_REQUEST["dates"]==2) $dur = $a["task_duration_ir"] ;
		else $dur = $a["task_duration"];
		/*if ($type == 24) {
