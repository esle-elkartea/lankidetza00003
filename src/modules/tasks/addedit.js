// $Id: addedit.js,v 1.16 2006/03/27 11:56:21 Attest sw-libre@attest.es Exp $
...
function setTasksStartDate(form, datesForm) {
	...
	if (form.set_task_start_date.checked == true) {	
		...
		*/
			...
			datesForm.start_date.value = projTasksWithEndDates[max_id][0] ;
			datesForm.start_hour.value = '08';
			datesForm.start_minute.value = '00';
...
function addUser(form) {
	...
	var perc = form.percentage_assignment.value;
...
*/
function calcDuration(f , real ) {
	
	if (real==1) {
		var int_st_date = new String(f.task_start_date_ir.value); 
		var int_en_date = new String(f.task_end_date_ir.value);
	}
	else { 
	...
	if ( s > e ) {
		...
		if (real==1) f.task_duration_ir.value = Math.round(durn) +1;
		else f.task_duration.value = Math.round(durn) +1;
		return (1);
	}
...
function checkDates(form) {
		...
		if (!form.task_start_date.value) {
			...
			alert('Introduzca una fecha de inicio a la tarea.');
			return false;
		}
		if (!form.task_end_date.value) {
			
			alert('Introduzca una fecha fin a la tarea.');
			return false;
		}
		...
		if (form.task_start_date_ir.value > form.task_end_date_ir.value) {
			
			alert('La fecha fin REAL debe ser posterior a la fecha inicio REAL!');
			return false;	
		}
...
function saveDates(form) {
	...
	if (can_edit_time_information) {
		...
		if ( form.task_end_date.value.length > 3 ) {
			form.task_end_date.value += form.end_hour.value + form.end_minute.value;
		}else {
			form.task_end_date.value = form.task_start_date.value;
		
		}
		if ( form.task_end_date_ir.value.length <3 ) {
			form.task_end_date_ir.value = form.task_end_date.value;
		}
		if ( form.task_start_date_ir.value.length <3 ) {
				form.task_start_date_ir.value = form.task_start_date.value;
		}
	}
	if (calcDuration (document.datesFrm,1)==1) {	
	//ina
	return new Array('task_start_date', 'task_end_date','task_start_date_ir','task_end_date_ir','task_duration_ir'); }
	else 
		return false;
}
