<?php  /* TASKS $Id: vw_logs.php,v 1.24 2006/04/06 16:02:57 Attest sw-libre@attest.es Exp $ */
/* TASKS $Id: vw_logs.php,v 1.23 2005/03/14 02:18:16 ajdonnison Exp $ */
...
?>
function delIt2(id) {
	if (confirm( "<?php echo $AppUI->_('do Delete Log', UI_OUTPUT_JS)?>" )) {
...
$sql = "
SELECT task_log.*, user_username, departments.dept_name
FROM task_log
...
foreach ($logs as $row) {
	...
	if ($perms->checkModuleItem('task_log', 'edit', $task_id)) {
        ...
        $s .= "\n\t</td>";
	$s .= '<td nowrap="nowrap">'.($task_log_date ? substr($task_log_date->format( $df ),0,10) : '-').'</td>';
	...
	$s .= '<td width="100">'.$row["user_username"].( $row["dept_name"] ? ' / '.$row["dept_name"] : '' ).'</td>';
...
$s .= '<tr bgcolor="white" valign="top">';
$s .= '<td colspan="5" align="right">' . $AppUI->_('Total Hours') . ' =</td>';
...
//ina $s .= '<input type="submit" class="button" value="'.$AppUI->_('new log').'">';
$s .= '</form></td></tr>';
echo $s;
?>
</table>
