<?php /* ROLES $Id: index.php,v 1.9 2006/04/11 17:56:22 Attest sw-libre@attest.es Exp $ */
/* ROLES $Id: index.php,v 1.8 2005/04/01 20:51:51 gregorerhardt Exp $ */
$AppUI->savePlace();
...
?>
function delIt(id) {
	if (confirm( '<?echo $AppUI->_('¿Está seguro de querer eliminar este rol?')?>' )) {
		f = document.roleFrm;
		f.del.value = 1;
		f.role_id.value = id;
		f.submit();
...
function showRow( $role=null ) {
	...
	// edit form
		$s .= '<form name="roleFrm" method="post" action="?m=roles">'.$CR;
		...
		if (($canEdit )&&($id!=11)){
			$s .= '<a href="?m=roles&role_id='.$id.'" title="'.$AppUI->_('edit').'">';
			$s .= dPshowImage('./images/icons/stock_edit-16.png');
			$s .= "</a><a href='?m=roles&a=viewrole&role_id=$id&tab=1' title='".$AppUI->_('edit permissions')."'>";
