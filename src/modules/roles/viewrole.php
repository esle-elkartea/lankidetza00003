<?php /* ROLES $Id: viewrole.php,v 1.1 2006/02/14 18:35:23 Attest sw-libre@attest.es Exp $ */
...
if (! is_array($role)) {
	...
	$titleBlock->addCrumb("?m=roles", "role list");
	$titleBlcok->show();
} else {
	$titleBlock = new CTitleBlock('Ver Rol', 'main-settings.png', $m, "$m.$a");
	$titleBlock->addCrumb("?m=roles", "lista de roles");
	$titleBlock->show();
...
<?php
	$tabBox = new CTabBox("?m=roles&a=viewrole&role_id=$role_id", "./modules/roles/", $tab );
